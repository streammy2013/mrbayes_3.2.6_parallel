...
(c) Configure CPU Parallel MrBayes specifying the building configure:
    ...
    Configure: 
	If using MPI:
		./configure 每enable-mpi 每with-beagle=no
	else:
		./configure 每with-beagle=no
...
otherwise: mpirun 每n (number of process) ./mb (your data address)
For example, if ＆mb＊ is in the same directory as your data: mpirun 每n 4 ./mb data/dataset1.nex 
